...
export const Route = createFileRoute("/support")({
  head: () => ({
    meta: [
      { title: "Support Center — Great Commission" },
      { name: "description", content: "Support needs, monthly budgets, and special projects for every missionary." },
    ],
  }),
...
function SupportPage() {
  const totalNeeded = missionaries.reduce((s, m) => s + m.monthlySupportNeeded, 0);
  const totalReceived = missionaries.reduce((s, m) => s + m.supportReceived, 0);
  const percent = Math.round((totalReceived / totalNeeded) * 100);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="card-soft gradient-mission p-8 text-white shadow-lift">
        <div className="flex flex-wrap items-baseline justify-between gap-4">
          ...
          <div className="text-right">
            ...
            <div className="mt-2 font-display text-4xl font-semibold">{percent}%</div>
          </div>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {missionaries.map((m) => {
          const pct = Math.round((m.supportReceived / m.monthlySupportNeeded) * 100);
          const remaining = Math.max(0, m.monthlySupportNeeded - m.supportReceived);
          return (
            <Card key={m.id} className="card-soft p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-sm text-muted-foreground">{m.missionField}</p>
                  <Progress value={pct} className="mt-2 h-2" />
                  <div className="mt-2 flex flex-wrap gap-2 text-xs">
                    <span className="rounded-full bg-secondary/15 px-2 py-0.5 text-secondary">Received ₱{m.supportReceived.toLocaleString()}</span>
                    {remaining > 0 ? (
